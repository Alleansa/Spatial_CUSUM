...
one_iter<-function(N.grid,r,k,Dist_matrix,scale=0, shift =0, threshold=0.4627){
  ...
  if(mean(CUSUM(test_seq/mad(test_seq)))>threshold) {
    ...
    grid.point$boundary_score<-nn_mean(grid.point$detection,Dist_matrix,r)
...
stopCluster(cl)
saveRDS(colMeans(aa1), "/home/xinzhang/spatial_CUSUM/diagram_boundary/aa1.rds")
...
stopCluster(cl)
saveRDS(colMeans(aa2), "/home/xinzhang/spatial_CUSUM/diagram_boundary/aa2.rds")
...
stopCluster(cl)
saveRDS(colMeans(aa3), "/home/xinzhang/spatial_CUSUM/diagram_boundary/aa3.rds")

library(ggplot2)
aa1<-readRDS("~/desktop/aa1.rds")
m1<-(aa1>0.7)
N.grid<-100
grid.point = expand.grid(x=seq(0,1,by=1/N.grid),y=seq(0,1,by=1/N.grid))
