...
aa_1=foreach(m_time=1:100,.combine=rbind,
            ...
            {
              multi_iter(N.grid=100,r=0.15,k=5,shift=1,scale=0,times=1,Dist_matrix = D)
            }
stopCluster(cl)
saveRDS(aa_1, "/work/STAT/xinzhang/CUSUM/Results_H1/3/aa_1.rds")
...
aa_2=foreach(m_time=1:100,.combine=rbind,
             .packages=c("RandomFields","foreach"))%dopar%
             {
               multi_iter(N.grid=100,r=0.15,k=5,shift=2,scale=0,times=1,Dist_matrix = D)
             }
stopCluster(cl)
saveRDS(aa_2, "/work/STAT/xinzhang/CUSUM/Results_H1/3/aa_2.rds")
...
aa_3=foreach(m_time=1:100,.combine=rbind,
             .packages=c("RandomFields","foreach"))%dopar%
             {
               multi_iter(N.grid=100,r=0.15,k=5,shift=3,scale=0,times=1,Dist_matrix = D)
             }
stopCluster(cl)
saveRDS(aa_3, "/work/STAT/xinzhang/CUSUM/Results_H1/3/aa_3.rds")
...
aa_5=foreach(m_time=1:100,.combine=rbind,
             ...
             {
               multi_iter(N.grid=100,r=0.15,k=5,shift=5,scale=0,times=1,Dist_matrix = D)
             }
stopCluster(cl)
saveRDS(aa_5, "/work/STAT/xinzhang/CUSUM/Results_H1/3/aa_5.rds")
