...
match_wrong<-function(pvalues,status,x,y,miss_report,window=5,n=101){
...
# res1<-FDRLMethod(pvalues,data$x,data$y,window=5,alpha=0.05)
# res2<-FDRLMethod(pvalues,data$x,data$y,window=9,alpha=0.05)
# data$detection_fdrl1<-res1$ind
# data$detection_fdrl2<-res2$ind
# ggplot()+geom_point(data=data, aes(x = x, y = y,colour=detection_fdrl1))
# ggplot()+geom_point(data=data, aes(x = x, y = y,colour=detection_fdrl2))
