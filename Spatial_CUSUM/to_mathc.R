...
aa_2_1=foreach(m_time=1:100,.combine=rbind,
              .packages=c("RandomFields","foreach","lpdensity"),
              .errorhandling="remove")%dopar%
              {
                multi_iter(N.grid=100,shift=1.5,scale=0,times=50,k=5,alpha=0.17)
              }
stopCluster(cl)

cl=makeCluster(13)
registerDoParallel(cl)
aa_2_2=foreach(m_time=1:100,.combine=rbind,
              .packages=c("RandomFields","foreach","lpdensity"),
              .errorhandling="remove")%dopar%
              {
                multi_iter(N.grid=100,shift=1.5,scale=0,times=50,k=5,alpha=0.25)
              }
stopCluster(cl)

cl=makeCluster(13)
registerDoParallel(cl)
aa_2_3=foreach(m_time=1:100,.combine=rbind,
               .packages=c("RandomFields","foreach","lpdensity"),
               .errorhandling="remove")%dopar%
               {
                 multi_iter(N.grid=100,shift=1.5,scale=0,times=50,k=5,alpha=0.2)
               }
stopCluster(cl)
