...
multi_iter<-function(N.grid=100,shift=0,scale=0,times=10,k=5,alpha=0.05){
  ...
  result<-c(miss_report/sum(grid.point$Status),wrong_report/sum(1-grid.point$Status),wrong_report/sum(grid.point$detection),
         fdrl_miss/sum(grid.point$Status),fdrl_wrong/sum(1-grid.point$Status),fdrl_fdr)
  return(result)
...
aa_05=foreach(m_time=1:100,.combine=rbind,
             ...
             {
               multi_iter(N.grid=100,shift=0.6,scale=0,times=50,k=10,alpha=0.05)
             }
stopCluster(cl)
...
aa_08=foreach(m_time=1:100,.combine=rbind,
             ...
             .errorhandling="remove")%dopar%
             {
               multi_iter(N.grid=100,shift=0.8,scale=0,times=50,k=10,alpha=0.05)
             }
stopCluster(cl)
...
aa_1=foreach(m_time=1:100,.combine=rbind,
             ...
             .errorhandling="remove")%dopar%
             {
               multi_iter(N.grid=100,shift=1,scale=0,times=50,k=10,alpha=0.05)
             }
stopCluster(cl)
...
aa_15=foreach(m_time=1:100,.combine=rbind,
             ...
             .errorhandling="remove")%dopar%
             {
               multi_iter(N.grid=100,shift=1.5,scale=0,times=50,k=10,alpha=0.05)
             }
stopCluster(cl)
...
aa_2=foreach(m_time=1:100,.combine=rbind,
             ...
             .errorhandling="remove")%dopar%
             {
               multi_iter(N.grid=100,shift=2,scale=0,times=50,k=10,alpha=0.05)
             }
stopCluster(cl)
saveRDS(aa_2,"/vol/data/zhuz/xinzhang/spatial_cusum/aa_2.rds")

bb = subset(grid.point,bound == 1)
aa = geom_point(data = bb,aes(x=x,y=y),color="red")
a+aa
a
